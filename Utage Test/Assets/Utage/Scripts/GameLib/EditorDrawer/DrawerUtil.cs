...
namespace Utage
{
	...
	public class DrawerUtil
	{
		...
		public static object CallFunction(SerializedProperty property, string functionName, object[] args = null)
		{
			...
			//メソッドを名前で検索
			var method = type.GetMethod(functionName, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);              //メソッドを名前で検索
			if (method != null) return method.Invoke(obj, args);                        //メソッド呼び出し

			//プロパティを名前で検索
			var prop = type.GetProperty(functionName, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.GetProperty);
