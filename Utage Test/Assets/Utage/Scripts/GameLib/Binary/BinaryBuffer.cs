...
namespace Utage
{
	...
	public class BinaryBuffer
	{
		...
		//バイナリ化して書き込み
		static public void Write(BinaryWriter writer, List<IBinaryIO> ioList)
		{
			BinaryBuffer data = new BinaryBuffer();
			data.MakeBuffer(ioList);
			data.Write(writer);
		}

		//バイナリ化して読み込み
		static public void Read(BinaryReader reader, List<IBinaryIO> ioList)
		{
			BinaryBuffer data = new BinaryBuffer();
			data.Read(reader);
		...
		{
			BinaryBuffer clone = new BinaryBuffer();
