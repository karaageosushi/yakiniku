...
namespace Utage
{
	...
	public class AdvUguiLoadGraphicFile : MonoBehaviour
	{
		...
		/// 描画コンポーネント
		/// </summary>
		Graphic GraphicComponent { get; set; }

		/// <summary>
		/// ロードするファイル
		/// </summary>
		AssetFile File { get; set; }

		//グラフィック情報
		AdvGraphicInfo GraphicInfo { get; set; }
		...
		T ChangeGraphicComponent<T>() where T : Graphic
		{
			//まだ設定されてないならGetする
		...
		void InitSize(Vector2 resouceSize)
		{
